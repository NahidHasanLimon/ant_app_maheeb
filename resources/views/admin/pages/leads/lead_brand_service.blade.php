...
@section('content')
    ...
    <div class="top_text">
        <h1>Brand Product or Services</h1>
    </div>
            ...
            <div style="border: none; border-radius:25px " class="card">
                    ...
                    <div class="col-md-4 d-flex align-items-center justify-content-between justify-content-md-start mb-3 mb-md-0">
                        <div class="align-items-center m-4 d-inline-flex">
                            ...
                            <a class="btn btn-primary btn-sm add_btn" href="" data-toggle="modal"
                               data-target="#add_holiday_modal" id="add_company_btn">
                ...
                <div style="position: relative" class="card-body">
                    ...
                    {{--<button class="vieweditButoon" data-toggle="modal"--}}
                        ...
                        {{--<a href="#" class="">--}}
                            {{--<i class="fa fa-plus my-float"> <span class="btn_txt">Add new Brand Product/Services</span></i>--}}
                            ...
                            <div id="maheeb" class="row">
                                <div class="col-12">
                                    <div class="table-responsive">
                                        <table id="brand_prod_service_table" class="table holiday_table">
                                            ...
                                            @else
                                                ...
                                                @foreach ($items as $item)
                                                    <tr class="unqcompany{{$item->id}}">
                                                        ...
                                                        <td>{{$item->lead_brand->lead_company->lead_company_name}}</td>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    {{--<h5 class="modal-title" id="exampleModalLabel">Add New Brand Product/Service</h5>--}}
                    ...
                    <div style="border: none" class="card">
                        <div class="card-title">
                            <div class="preview"><i class="" aria-hidden="true"></i>
                                <h5 class="text-center font-weight-bolder" id="">Add New Brand Product/Service</h5>
                            </div>
                        </div>
                                        ...
                                        <fieldset>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select class="form-control" id="psId" name="psName">
                                                    ...
                                                    @foreach($leadprodServices as $service)
                                                        <option value="{{$service->id}}">{{$service->lead_product_or_service_name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            {{--<input id="sub_dept_id" class="form-control" name="sub_dept_name"--}}
                                            {{--type="text"--}}
...
@section('pageLevelScript')
    <script type="text/javascript">
        ...
        var brand_prod_service_table = $('#brand_prod_service_table').DataTable({
        ...
        $('#search_datatable_input').on('keyup', function () {
            brand_prod_service_table.search(this.value).draw();
        ...
        jQuery(function ($) {
            ...
            $('ul a').each(function () {
                if (this.href === path) {
                    ...
                    $("#manage_brand_product_service").css({
        ...
        jQuery(document).ready(function () {
            jQuery('select[name="cmpName"]').on('change', function () {
                ...
                if (countryID) {
                    jQuery.ajax({
                        url: 'brand-name/getLeadBrand/' + countryID,
                        type: "GET",
                        dataType: "json",
                        success: function (data) {
                            ...
                            jQuery.each(data, function (key, value) {
                                ...
                                $('select[name="deptName"]').append('<option value="' + value.id + '">' + value.lead_brand_name + '</option>');
                            });
                        }
                    });
                }
        ...
        var config = {
            routes: {
                brand_product_service_store: "{!! route('brand-product-service.post') !!}",
                brand_product_service_delete: "{!! route('brand-product-service.delete') !!}",
                brand_product_service_detail: "{!! route('brand-product-service.detail') !!}",
                brand_product_service_update: "{!! route('brand-product-service.update') !!}",
        ...
        $(document).on('submit', '#add_holiday_form', function (event) {
            ...
            $.ajax({
                url: config.routes.brand_product_service_store,
                method: "POST",
                ...
                success: function (data) {
                    if (data.success == true) {
                        ...
                        var rowCount = $('#brand_prod_service_table >tbody >tr').length + 1;


                        brand_prod_service_table.row.add([

                            '' + data.lc.lead_company_name   + '',
                            '' + data.item.lead_brand.lead_brand_name  + '',
                            '' + data.item.lead_product_or_service.lead_product_or_service_name + '',
...
//
//
//               <td>` + data.lc.lead_company_name + ` </td>
//             <td>` + data.item.lead_brand.lead_brand_name + ` </td>
//
//
//             <td>` + data.item.lead_product_or_service.lead_product_or_service_name + ` </td>
//
//
//
// <td>
...
//             </tr>`);
        ...
        $(document).on('click', '.deletecompany', function (e) {
            ...
            if (confirm('Are You Sure to Delete ?')) {
                $.ajax({
                    url: config.routes.brand_product_service_delete,
                    type: "get",
                    data: {
                        id: id
                    },
        ...
        $(document).on('click', '.editcompany', function (e) {
            ...
            $.ajax({
                url: config.routes.brand_product_service_detail,
                type: "get",
                data: {
                    id: id,
                    type: type,
                ...
                success: function (data) {
                    ...
                    $('#company_edit_form').find('#cmpId').val(data.lc.id);
                    ...
                    $('#company_edit_form').find('#psId').val(data.lps.id);
        ...
        $(document).on('submit', '#company_edit_form', function (e) {
            ...
            $.ajax({
                url: config.routes.brand_product_service_update,
                method: "POST",
                data: new FormData(this),
                dataType: 'JSON',
                contentType: false,
