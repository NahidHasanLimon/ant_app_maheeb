...
@section('content')
    ...
    <div class="top_text">
        <h1>Manage Project Status Category</h1>
    </div>
                    ...
                    <div class="col-md-4 d-flex align-items-center justify-content-between justify-content-md-start mb-3 mb-md-0">
                        <div class="align-items-center m-4 d-inline-flex">
                            ...
                            <a class="btn btn-primary btn-sm add_btn" href="" data-toggle="modal"
                               data-target="#projects_status_category_modal" id="add_company_btn">
                                <i class="fa fa-plus"> Add Project Status Category</i>
                            </a>
                        </div>
                    </div>
                            ...
                            <div id="maheeb" class="row">
                                <div class="col-12">
                                    <div class="table-responsive">
                                        <table id="projects_status_category_table" class="table holiday_table">
                                            <thead>
                                            ...
                                            <tbody class="holiday_row" id="projects_status_category_table_append">
                                                ...
                                                @foreach ($items as $item)
                                                    <tr  class="unqcompany{{$item->id}}">
                                                        ...
                                                        <td>
                                                            ...
                                                            <a href="{{route('filtered-project-status-cat.index',$item->id)}}">{{$item->name}}</a>
                                                        ...
                                                        <td>
                                                            ...
                                                            <button type="button"
                                                                    ...
                                                                    class="btn edit_btn edit_pro_status_cat commonbtn"
                                                                    data-id="{{$item->id}}" data-toggle="modal"
                                                                    data-target="#exampleModal">
                                                                <i class="fa fa-pencil"></i>
                                                            </button>
                                                            <button type="button" data-id="{{$item->id}}"
                                                                    class="btn delete_btn delete_pro_status_cat commonbtn">
    ...
    <div class="modal fade" id="projects_status_category_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                    ...
                    <div class="card">
                        <div class="card-title">
                            <div class="preview"><i class="" aria-hidden="true"></i>
                                <h5 class="text-center font-weight-bolder" id="">Add Project Status Category</h5>
                            </div>
                        </div>
                        <div class="card-body text-center d-flex flex-column justify-content-center">
                            <div class="row">
                                <div class="col-lg-12">
                                    ...
                                    <form method="post" name="add_projects_status_category_form"
                                          id="add_projects_status_category_form">
                                        @csrf
                                        <fieldset>

                                            <div class="form-group">
                                                <label for="pro_cat_status_name">Project Status Category</label>
                                                <input id="pro_cat_status_name_id" class="form-control" name="name"
                                                       type="text"
                                                       placeholder="Enter Project Status Category.." required>
                                            </div>
    ...
    <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                ...
                <h5 class="text-center font-weight-bolder" id="">Edit Project Status Category</h5>
                <form method="post" name="projects_status_category_edit_form" id="projects_status_category_edit_form">
                    @csrf
                    <div class="modal-body">
...
@section('pageLevelScript')
    <script type="text/javascript">
        ...
        var projects_status_category_table = $('#projects_status_category_table').DataTable({
        ...
        $('#search_datatable_input').on('keyup', function () {
            projects_status_category_table.search(this.value).draw();
        ...
        jQuery(function ($) {
            ...
            $('ul a').each(function () {
                if (this.href === path) {
                    ...
                    $("#manage_project_status_category").css({
        ...
        var config = {
            routes: {
                projects_category_status_store: "{!! route('project-status-category.post') !!}",
                projects_status_category_delete: "{!! route('project-status-category.delete') !!}",
                projects_status_category_detail: "{!! route('project-status-category.detail') !!}",
                projects_status_category_update: "{!! route('project-status-category.update') !!}",

            }
        };

        $("#add_projects_status_category_form").validate({
            rules: {
                name: {
                    required: true,
                },
        ...
        $("#projects_status_category_edit_form").validate({
            rules: {
                name: {
                    required: true,
                },
        ...
        $(document).on('submit', '#add_projects_status_category_form', function (event) {
            event.preventDefault();
            $.ajax({
                url: config.routes.projects_category_status_store,
                method: "POST",
                ...
                success: function (data) {
                    if (data.success == true) {
                        ...
                        var rowCount = $('#projects_status_category_table >tbody >tr').length + 1;


                        $('#projects_status_category_table_append').append(`<tr class="unqcompany` + data.item.id + `">
...
<td>
                 <button type="button" class="btn edit_btn edit_pro_status_cat commonbtn" data-toggle="modal"

                 data-id="` + data.item.id + `"
                 data-target="#exampleModal">
                   <i class="fa fa-pencil"></i>
                 </button>
                <button type="button" class="btn  delete_btn delete_pro_status_cat commonbtn"
                data-id="` + data.item.id + `"
                >
                    <i class="fa fa-trash"></i>
                </button>
                        ...
                        // projects_status_category_table.row.add([
                        //
                        //     '' + data.item.name + '',
                        //     '' + data.item.definition + '',
                        //     '<button type="button" class="btn edit_btn edit_pro_status_cat commonbtn" data-toggle="modal" data-id="' + data.item.id +'" data-target="#exampleModal">   <i class="fa fa-pencil"></i>   </button><button type="button" class="btn delete_btn delete_pro_status_cat commonbtn" data-id="'+ data.item.id +'">   <i class="fa fa-trash"></i>   </button>'
                        //
                        //
                        // ]) ;
                        // projects_status_category_table.draw();
                        ...
                        setTimeout(function () {
                            $('#projects_status_category_modal').modal('hide');
                        }, 1500);
                        $('#add_projects_status_category_form').trigger('reset');
        ...
        $(document).on('click', '.delete_pro_status_cat', function (e) {
            e.preventDefault();
            var id = $(this).data('id');
            ...
            if (confirm('Are You Sure to Delete ?')) {
                $.ajax({
                    url: config.routes.projects_status_category_delete,
                    type: "get",
                    data: {
                        id: id
                    },
        ...
        $(document).on('click', '.edit_pro_status_cat', function (e) {
            ...
            console.log(id,si);
            $.ajax({
                url: config.routes.projects_status_category_detail,
                type: "get",
                data: {
                    id: id,
                    type: type,
                ...
                success: function (data) {
                    // console.log(data);
                    $('#projects_status_category_edit_form').find('#pro_cat_status_name_id').val(data.pro_status_cat.name);
                    $('#projects_status_category_edit_form').find('#definition_id').val(data.pro_status_cat.definition);
                    $('#projects_status_category_edit_form').find('#id').val(id);
                    $('#projects_status_category_edit_form').find('#si').val(si);
                },
                error: function(data, errorThrown)
                {
        ...
        $(document).on('submit', '#projects_status_category_edit_form', function (e) {
            e.preventDefault();
            var si = $('#projects_status_category_edit_form').find('#si').val();

            $.ajax({
                url: config.routes.projects_status_category_update,
                method: "POST",
                data: new FormData(this),
                dataType: 'JSON',
                contentType: false,
...
<td>
                ...
                >
                   <i class="fa fa-trash"></i>
                        ...
                        $('#projects_status_category_edit_form').trigger('reset');
                    } else {
