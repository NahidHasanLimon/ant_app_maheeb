...
@section('content')
    ...
    <div class="top_text">
        {{--        <h1>Manage SubDepartments</h1>--}}
        <h1>Manage {{$dept_name->department_name}}</h1>
    </div>
                        ...
                        <div class="align-items-center m-4 d-inline-flex">
                            <a class="btn btn-primary btn-sm add_btn" href="" data-toggle="modal"
                               data-target="#add_sub_department_modal" id="add_sub_department_btn">
                                <i class="fa fa-plus"> Add Sub Department</i>
                            </a>
                        </div>
                    </div>
                            ...
                            <div id="maheeb" class="row">
                                <div class="col-12">
                                    <div class="table-responsive">
                                        <table id="sub_department_table" class="table sub_department_table">
                    ...
                    <div style="border: none" class="card">
                        <div class="card-title">
                            <div class="preview"><i class="" aria-hidden="true"></i>
                                <h5 class="text-center font-weight-bolder" id="">Add New Sub-Department</h5>
                            </div>
                        </div>
...
{{--                                                        <option value="{{$dept_name->id}}" selected>{{$dept_name->department_name}}</option>--}}
{{--                                                </select>--}}
                                                <label for="department_name">Sub-department Name</label>
                                                <input id="deptId" class="form-control" name="deptName"
                                                       type="text"   value="{{$dept_name->department_name}}"
                                                       readonly disabled>
                                            </div>
                                            <div class="form-group">
                                                <label for="department_name">Sub-department Name</label>
                                                <input id="sub_dept_id" class="form-control" name="sub_dept_name"
                                                       type="text"
                                                       placeholder="Enter Sub Department Name.." required>
                                            </div>
                                            <input type="hidden" id="id" name="deptIdHidden" value="{{$dept_name->id}}" class="form-control">
    ...
    <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                ...
                <h5 class="text-center font-weight-bolder" id="">Edit SubDepartment</h5>
...
@section('pageLevelScript')
    <script type="text/javascript">
        var sub_department_table = $('#sub_department_table').DataTable({
            sDom: 'lrtip',
            "paging": false,
            fixedHeader: true
        });
        $('#search_datatable_input').on('keyup', function () {
            sub_department_table.search(this.value).draw();
        ...
        var config = {
            routes: {
                filtered_sub_dept_store: "{!! route('filteredSubDepartment.post') !!}",
                {{--sub_dept_store: "{!! route('subDepartment.post') !!}",--}}
                sub_dept_delete: "{!! route('subDepartment.delete') !!}",
                {{--                sub_dept_detail: "{!! route('subDepartment.detail') !!}",--}}
                filtered_sub_dept_detail: "{!! route('filtered.test') !!}",
                filtered_sub_dept_update: "{!! route('filteredSubDepartment.update') !!}",
        ...
        $(document).on('submit', '#add_holiday_form', function (event) {
            ...
            $.ajax({
                url: config.routes.filtered_sub_dept_store,
                method: "POST",
                ...
                success: function (data) {
                    if (data.success == true) {
                        ...
                        var rowCount = $('#sub_department_table >tbody >tr').length + 1;
                        $('#companyappend').append(`<tr class="unqcompany` + data.item.id + `">


                <td>` + data.item.department.department_name + ` </td>
                <td>` + data.item.sub_department_name + ` </td>
...
<td>
        ...
        $(document).on('click', '.deletecompany', function (e) {
            ...
            if (confirm('Are You Sure to Delete ?')) {
                $.ajax({
                    url: config.routes.sub_dept_delete,
                    type: "get",
                    data: {
                        id: id
                    },
        ...
        $(document).on('click', '.editcompany', function (e) {
            ...
            $.ajax({
                url: config.routes.filtered_sub_dept_detail,
                type: "get",
                data: {
                    id: id,
                    type: type,
                ...
                success: function (data) {
                    $('#company_edit_form').find('#deptId').val(data.department.department_name);
                    $('#company_edit_form').find('#sub_dept_id').val(data.subDept.sub_department_name);
        ...
        $(document).on('submit', '#company_edit_form', function (e) {
            ...
            $.ajax({
                url: config.routes.filtered_sub_dept_update,
                method: "POST",
                data: new FormData(this),
                dataType: 'JSON',
                contentType: false,
